...
class relatedArtistsPlaylistClass():
    ...
    def __init__(self):
        self.username = '22kpgi2vtrlebcei6eu37db7y'
        self.scope = 'playlist-read-collaborative playlist-modify-private playlist-modify-public playlist-read-private user-modify-playback-state user-read-currently-playing user-read-playback-state user-read-private user-read-email user-library-modify user-library-read user-follow-modify user-follow-read user-read-recently-played user-top-read streaming app-remote-control'

        try:
            self.token = util.prompt_for_user_token(self.username,self.scope,client_id='8ff0ccc6f1fb460e8fabbe33e0e42112',client_secret='03d0e4de0957434abcc60660045d7cfc',redirect_uri='https://www.google.com/')
        except:
            os.remove(f".cache-{self.username}")
        
        self.spotifyObject = spotipy.Spotify(auth=self.token)     # create object of type spotipy
        self.user = self.spotifyObject.current_user()             # get current app user
      

    def unclepostyPlaylist(self):
        searchResults = self.spotifyObject.search('Post+Malone', 1, 0, 'artist')     # search for selected artist object
        getArtist = searchResults['artists']['items'][0]
        artistId = getArtist['id']          # get slected artist's Id

        relatedArtists = self.spotifyObject.artist_related_artists(artistId)
        artists = relatedArtists['artists']     # search for related artists

        for x in artists:
            artistName = (x['name'])
            artistSearch = self.spotifyObject.search(artistName, 2, 0 ,'track')      # search all related artists
